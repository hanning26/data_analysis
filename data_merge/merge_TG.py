...
global ft_paths2 
global rt_paths2
global data_merge_paths
global invalid_folder_paths
global history_backup_paths
...
def path_init(path):
    ...
    global history_backup_paths
    paths = path  # 总的文件路径
    ft_paths1 = (paths + '/FT').replace('\\', '//')
    rt_paths1 = (paths + '/RT').replace('\\', '//')
    ft_paths2 = (paths + '/历史备份/FT').replace('\\', '//')
    rt_paths2 = (paths + '/历史备份/RT').replace('\\', '//')
    data_merge_paths = (paths + '/合并数据').replace('\\', '//')
    invalid_folder_paths = (paths + '/无效文件').replace('\\', '//')
    history_backup_paths = (paths + '/历史备份').replace('\\', '//')
    ft = (r'\\172.17.1.212\FCTRecords\106-LED\标普设备\测试数据\RT')
...
def create_time_folder(path):
    ...
    # 时：分：秒
    hour = dt.datetime.now().strftime("%H_%M_%S")
    # print(now_time + "\n" + day + "\n" + hour)
    # 创建时间到月的文件夹
...
def exception_file_recognition(file_path):
    ...
    if ft_name_file.count('_') != 7:
        ...
        if Path(invalid_foldername.replace('\\', '/') + '/' + ft_name_file + '.csv').exists():
            os.remove(invalid_foldername + '/' + ft_name_file + '.csv')
        shutil.move(file_path, invalid_foldername)
        return 1
    if (file_path.rfind('副本') > 0) or (file_path.upper().rfind('XLS') > 0):
        logging.error(ft_name_file + ':文件名格式错误')
        invalid_foldername, invalid_time = create_time_folder(invalid_folder_paths)
        if Path(invalid_foldername.replace('\\', '/') + '/' + ft_name_file + '.csv').exists():
            os.remove(invalid_foldername + '/' + ft_name_file + '.csv')
...
# 处理原始FT/RT数据，并在历史备份\\FT/RT文件夹中创建每月的文件夹并将文件按月份区分
def year_folder():
    ...
    try:    
        ...
        for ft_csvFile in ft_csvFiles:
            ...
            # 提取FT文件到月份文件夹,先判断文件是否存在，存在则删除
            if Path(year_ft_path.replace('\\', '/') + '/' + ft_name_file + '.csv').exists():
                os.remove((year_ft_path + '/' + ft_name_file + '.csv'))
        ...
        for rt_csvFile in rt_csvFiles:
            ...
            # 提取RT文件到月份文件夹,先判断文件是否存在，存在则删除
            if Path(year_rt_path.replace('\\', '/') + '/' + rt_name_file + '.csv').exists():
                os.remove(year_rt_path + '/' + rt_name_file + '.csv')
            shutil.move(rt_csvFile, year_rt_path)

        if ft_csvFiles == [] and rt_csvFiles == []:
            return 1
...
def get_data_type():
    ...
    try:    
        ...
        rt_num = 0
        ft_batch_count = {}
        rt_batch_count = {}    # 确认FT数据是否有与其对应的RT数据
        ft_rt_map1 = {}  # 找到所有FT数据与其对应的RT数据 
        ft_rt_map2 = {}  # 存放RT数据的测试代号
        # 将RT数据无FT对应的数据移动到无效数据
        for rt_batch in rt_batch_names:
            for ft_batch in ft_batch_names: 
                if (ft_batch == rt_batch):
                    rt_num += 1
            rt_batch_count[rt_batch] = rt_num
            rt_num = 0   # num变量必须清零
        # shutil.copy语句最好还程shutil.move，否则语句还保留在FT中
        for rt_name, rt_value in rt_names_dict.items():
            if (rt_batch_count[rt_value['测试批号']] == 0):
                invalid_foldername, invalid_time = create_time_folder(invalid_folder_paths)
                shutil.copy(rt_name, invalid_foldername)
        ...
        for ft_batch in ft_batch_names:
            for rt_batch in rt_batch_names: 
                if (rt_batch == ft_batch):
                    ft_num += 1
            ft_batch_count[ft_batch] = ft_num
            num = 0   # num变量必须清零
        for ft_name, ft_value in ft_names_dict.items():
            if (ft_batch_count[ft_value['测试批号']] == 0):
...
def init():
    ...
    global rt_now_csvFiles1, rt_last_csvFiles1, rt_now_csvFiles2, rt_last_csvFiles2
    try:
        app_log()
        year_folder()
        get_data_type()
        ...
        create_folder(ft_now_path1)
        create_folder(ft_last_path1)
        create_folder(rt_now_path1)
        create_folder(rt_last_path1)
        # 获取当前FT文件夹下的文件名
        # 拼接FT文件名和地址
...
def merge_main():
    ...
    if year_folder() != 1:
        ...
        create_folder(rt_now_path2)
        create_folder(rt_last_path2)
        create_folder(ft_now_path2)
        create_folder(ft_last_path2)
        # 拼接文件名和地址
        ...
        try: 
            if rt_differ_file_lists != [] or ft_differ_file_lists != []:          
                # 处理测试数据
                ft_batch_name = []
                ft_batch_names = []
